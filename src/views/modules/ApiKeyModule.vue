...
const newApiKey = ref<Partial<ApiKey>>({
  name: '',
  endpoint: '',
  expiration: '',
});
...
async function updateApiKey() {
  ...
  try {
    await axios.put(
      `http://localhost:3001/apiKeys/${editingApiKey.value.id}`,
      editingApiKey.value
    );
...
<template>
  <div class="p-6">
    <div class="max-w-6xl mx-auto">
      ...
      <!-- Action Button -->
      <div class="mb-6">
        <button @click="openNewKeyModal" class="btn btn-primary">Create API Key</button>
      </div>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="apiKey in apiKeys" :key="apiKey.id">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button @click="openEditModal(apiKey)" class="text-primary hover:text-primary-dark mr-3">
                  Edit
                </button>
                <button @click="deleteApiKey(apiKey.id)" class="text-red-600 hover:text-red-900">
                  Delete
                </button>
              </td>
            </tr>
        ...
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <input v-model="newApiKey.expiration" type="date" class="input" />
            </div>
          </div>

          <div class="flex justify-end space-x-2 mt-6">
            <button @click="showNewKeyModal = false" class="btn btn-secondary">Cancel</button>
            <button @click="createApiKey" class="btn btn-primary" :disabled="loading">
...
<!-- Edit API Key Modal -->
<div
      v-if="showEditKeyModal"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
    >
      <div class="bg-white rounded-lg p-6 w-full max-w-md" v-if="editingApiKey">
        <!-- Only render the content if editingApiKey is defined -->
        <h2 class="text-xl font-bold mb-4">Edit API Key</h2>

        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">API Name</label>
            <input v-model="editingApiKey.name" type="text" class="input" />
          </div>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Expiration Date</label>
            <input v-model="editingApiKey.expiration" type="date" class="input" />
          </div>
        ...
        <div class="flex justify-end space-x-2 mt-6">
          <button @click="showEditKeyModal = false" class="btn btn-secondary">Cancel</button>
          <button @click="updateApiKey" class="btn btn-primary" :disabled="loading">
