...
// Define the Endpoint interface (inside EndpointModule.vue)
interface Endpoint {
  id: number;
  name: string;
  type: string;
...
// Define the New Endpoint interface (inside EndpointModule.vue)
...
//Use NewEndpoint
const newEndpoint = ref<NewEndpoint>({
  name: '',
  destination: '',
  apiKey: '',
  type: 'http',
...
async function updateEndpoint() {
  ...
  try {
    await axios.put(
      `http://localhost:3001/endpoints/${editingEndpoint.value.id}`,
...
<template>
  <div class="p-6">
    <div class="max-w-6xl mx-auto">
      ...
      <!-- Action Button -->
      <div class="mb-6">
        <button @click="openNewEndpointModal" class="btn btn-primary">
      ...
      >
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
          ...
          <div class="flex justify-end space-x-2 mt-6">
            <button @click="showNewEndpointModal = false" class="btn btn-secondary">
              Cancel
            </button>
            <button @click="createEndpoint" class="btn btn-primary" :disabled="loading">
      ...
      <!-- Edit Endpoint Modal -->
      <div
        v-if="showEditEndpointModal"
        class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
      >
        <!--add v-if check -->
        <div v-if="editingEndpoint" class="bg-white rounded-lg p-6 w-full max-w-md">
          <h2 class="text-xl font-bold mb-4">Edit Endpoint</h2>

          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Endpoint Name</label>
              <input v-model="editingEndpoint.name" type="text" class="input" />
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Setting Destination</label>
              <input v-model="editingEndpoint.destination" type="text" class="input" />
            </div>

            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">API Key</label>
              <input v-model="editingEndpoint.apiKey" type="text" class="input" />
            </div>
          </div>

          <div class="flex justify-end space-x-2 mt-6">
            <button @click="showEditEndpointModal = false;editingEndpoint=null;" class="btn btn-secondary">
              Cancel
            </button>
            <button @click="updateEndpoint" class="btn btn-primary" :disabled="loading">
